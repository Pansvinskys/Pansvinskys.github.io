...
function Obstaculos(){
 ...
 for(j=0;j<=5;j++){
  for(i=0;i<=10;i++){
   if(mapa[j][i]=="x"){
    this.cubo1[j*10+i]=new THREE.Mesh(new THREE.BoxGeometry(2,2,2),new THREE.MeshBasicMaterial({map:textura}));
    this.cubo1[j*10+i].position.x=-10+2*i;
    this.cubo1[j*10+i].position.z=-10+2*j;
